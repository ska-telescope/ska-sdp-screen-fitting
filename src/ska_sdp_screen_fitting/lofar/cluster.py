...
def get_fast_solve_intervals(
    ...
    if mem_gb / gb_per_sol < 1.0:
        ...
        if solint < target_timestep:
            log.warn(
                "Not enough memory available "  # pylint: disable=C0209
                "for fast-phase solve. Reducing"
                " solution time interval from %d to %d time slots",
                target_timestep,
                solint,
            )
    else:
...
def get_slow_solve_intervals(
    ...
    # chunk size (since the solve is parallelized over the channels of a chunk,
    # so larger chunks require more memory). Therefore, we also need to ensure
    # that the chunk size works with the available memory.
    target_numchunks = np.ceil(cluster_parset["max_nodes"] / numobs)
    samples_per_chunk = int(np.ceil(numsamples / target_numchunks))
