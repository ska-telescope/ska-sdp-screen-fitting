...
if __name__ == "__main__":
    ...
    f = h5py.File("../resources/fast_phase_6.h5parm", "r")
    ...
    radec_coord = processing_utils.read_patch_list("../resources/skymodel.txt", f, "phase000")
    w = wcs.WCS(kl_cube.header)
    [coord_patch_x, coord_patch_y] = processing_utils.get_patch_coordinates(radec_coord, w)
    ...
    funcs = [np.cos, np.sin]
    val = funcs[polarization_idx%2](phase_corrected) * ampl[:, :, :, :, polarization_idx//2]
    ...
    [colormap_min_val, colormap_max_val] = get_boundaries(kl_cube, voronoi_cube, val, val,
                                                          time, freq_offset,
                                                          antenna,polarization_idx)

    fig, ax = plt.subplots(1,2, figsize=(15, 15))
    ax[0].imshow(kl_cube.data[time, freq_offset, antenna, polarization_idx, :, :], vmin=colormap_min_val, vmax=colormap_max_val);
    ax[0].scatter(coord_patch_x, coord_patch_y, c=val[time, freq_offset, antenna, : ], vmin=colormap_min_val, vmax=colormap_max_val,edgecolors='black', linewidth=0.5, s=150);
    ax[0].set_title("Karhunen Loève screens")
    ax[1].imshow(voronoi_cube.data[time, freq_offset, antenna, polarization_idx, :, :], vmin=colormap_min_val, vmax=colormap_max_val);
    ax[1].scatter(coord_patch_x, coord_patch_y, c=val[time, freq_offset, antenna, : ], vmin=colormap_min_val, vmax=colormap_max_val,edgecolors='black', linewidth=0.5, s=150);
